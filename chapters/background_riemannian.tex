...
A map \(F: X\to Y\) is said to be \emphmarginnote{equivariant} with respect to \(G\) if \[F(g \cdot x) = g \cdot F(x) \quad \text{for all } x\in X, \; g\in G\].
...
where \(V_j = g_{ij}V^i\).
This is the motivation for the notation \(g^\vsda\), as this operation is said to \emphmarginnote{lower an index} of the coefficients of the vector field in charts.
...
We can then (with care) use mathematical tools that apply only to Eucldiean space and use them for analysis on manifolds.
