...
Incoperating the geometry of the manifolds that data lives on provides a more accurate and insightful representation of the underlying data, and allows machine learning algorithms to perform better.
...
\begin{figure}[h]
	% \includegraphics{figures/diagrams/boundary_charts2.pdf} % \includegraphics{figures/diagrams/circle_atlas.pdf}
...
% \todo[inline]{Some examples of when this is useful}
...
% 	\caption{A coordinate chart \((U, \phi)\) of the sphere, \(\c{S}_2\).}
% \end{figure}
...
For generic tensor fields we can't define the pushforward, but we can define, the \emphmarginnote{pullback of a covaraint tensor field} pointwise.
...
% The thoery of representations if crucial to the development of equivariant nerual network architeturces, for example \todo{weiler, EMLP}.


% % \section{Frame bundles}
% % Previously we saw how we can define a \emphhyperref{frame}{def:coordinate_frame} as the basis of a tangent space, and that under changes in charts the coefficients of a tangent vector expressed in those charts can be related by an element of \(\groupGL\), a \emphhyperref{change of coordinate}{def:change of coordinates}.

% % One way to think of a frame \(f\) at a point \(p\in X\) is as a linear map \(f: \R^n \to T_pX\), taking coefficients of a vector in the framre and turning them into the vector itself. We can define the action of \(\groupGL\) on a frame by composition with the group action,
% % \[
% % 	(g \cdot f)(v) = f( g \cdot v), \quad g\in \groupGL ,\;\v\in\R^n
% % .\]
% % This action is \emphhyperref{free}{} and \emphhyperref{transitive}{}.
% % This implies that the \emphmarginnote{space of frames at a point}, \(F_p\), is homeomorphic to \(\groupGL\), but is not itself a group as we have no preferred "identity" frame.

% % We can define the \emphmarginnote{frame bundle of the tangent space} of a manifold as the disjoint union of the spaces of frames at each point,
% % \[
% % 	F(X) = \coprod_{p\in X} F_p
% % ,\]
% % and similar to tensor bundles, it inherits a natural topology and bundle structure from \(X\).
% % In addition, it inherits the group action of \(\groupGL\), acting on a point in the frame bundle \((p, f)\in F(X)\) by 
% % \[g\cdot (p, f) = (p, g\cdot f),\] for which the orbits are clearly \(F_p\).
% % The addition of this group action to the fibre bundle structure makes the frame bundle an example of a \emphmarginnote{principle \(G\) bundle}.

% % It is very rare that smooth sections of the frame bundle exist, and is equivalent to the tangent bundle being \emphhyperref{parallelisable}{}.

% % For general smooth manifolds, the frame bundle is a \(\groupGL\) bundle. 
% % With the addition of additional structure we can sometimes restrict the space of frames in the frame bundle of manifold, and the result is a principle \(G\)-bundle where \(G \subset \groupGL\).
% % This is known as \emphmarginnote{reducing the structure group}.
% % This is not always possible, but some examples include:
% % \begin{itemize}
% % 	\item If we have a notion of an inner product on \(T_pX\), we can chose only orthonormal frames, resulting in a \(\groupO\)-principle bundle.
% % 	\item If it is possible to choose a consistent \emphmarginnote{orientation}
% % \end{itemize}

% % \subsection{Associated vector bundles}

% % The vector bundles we have encountered so far in \cref{sec:tensor_bundle} can also be constructed as \emph{associated vector bundles} of the frame bundle.
...
\subsection{Tanget-cotangent isomorphism}
...
% \todo{fix the latex commented here}
...
\begin{theorem}[Nash embedding theorem]
	...
	Then there exists a \(C^k\) isometric embededding of \(X\) into \(\R^m\), \(f \in C^k(X, \R^m)\), such that \(g = f^*g_e\), where \(g_e\) is the Euclidean metric on \(\R^m\), and \(m \geq n(n+1)(3n+11)/2\).
\end{theorem}
...
This, or any other volume form \marginnote{volume forms define measures} \(\omega\), defines a \emphhyperref{measure in the measure-theoreic sense}{def:measure} of the \emphhyperref{Borel \(\sigma\)-algebra}{def:borel_sigma_algebra} of the open sets on the manifold.
...
\begin{theorem}[Riesz-Markov-Kakutani representaiton theorem]
	...
	Let \(X\) be a locally compact Haussdorff space, and let \(\Lambda\) be a positive linear functional on \(C(X)\). Then there exists a \(\sigma\)-algebra, \(\sigalg{X}\), on \(X\) containing all open sets on \(X\), and there exists a unique positive measure \(\mu\) on \(\sigalg{X}\) which represents \(\Lambda\) in the sense that:
	\begin{enumerate}
...
For each element of the dual lattic there are two eigenfunction, given by \[\phi_{1, \lambda}(x) = \sin(2 \pi \langle x, y \rangle)\] and \[\phi_{2, \lambda}(x) =  \cos(2 \pi \langle x, y \rangle).\]
