...
\Cref{sec:tensor_spaces} introduces tangent, cotanget, and tensor spaces at each point on the manifold, key objects in the study of differentiation on manifolds at a point.
\Cref{sec:tensor_bundle} intriduces tangent, cotanget, and tensor bundles, structures that attach elemnts of tangent, cotanget, and tensor spaces to each point on the manifold.
\Cref{sec:lie_groups}, explores Lie groups, symmetry groups that are also themselves manifolds.

Fianlly, in \cref{sec:riemannian_manifolds}, we delve into Riemannian manifolds and metrics, which introduce a notion of distance and curvature, enabling us to explore the intrinsic geometry of the underlying data.
\Cref{sec:integration} studies how to do integration on manifolds. We build up the notions of differnetial forms, the types of objects that we can integrate on manifolds, and orientations.
\Cref{sec:connections} introduces connections, a way of differentiating tensors, how they lead to geodesics, straight paths on manifolds, and parallel transport, a method for comparing tanget vectors at different points on a manifold.
...
If we can further reduce every open cover to having a \emph{finite} sub-cover, then we say the sapce is \emphmarginnote{compact\label{def:compact}}. 
...
The \emphmarginnote{interior of a manifold with boundary}, \(\inte X\), is an \(n\)-dimension topologicl manifold \emph{without boundary}.
The \emphmarginnote{boundary of a manifold with boundary}, \(\partial X\), is an \(n-1\)-dimension topological manifold \emph{without boundary}, and typically properties of \(\partial X\) are \emph{induced} by corresponding properties on \(\inte X\).
...
A very common strategy, both when defining properties and proving theorems, on manifolds, is to break apart the manifold into a \emphhyperref{open cover}{def:manifold_cover}, define or prove things locally, and then using for example the \emphhyperref{gluing lemma}{lemma:continuous_gluing}, patch the result back together.
...
We called another basis \emphmarginnote{positively oriented} if the determinant of the transition matrix \(\lambda^j_i\) is positive, and \emphmarginnote{negativly oriented} if this is negative.
...
If the volume form \(\omega\) applied to an ordered basis \(e_1, ..., e_n\) is positive then the basis is positivly oriented, otherwise it is negativly oriented.
...
\begin{figure}[h] \includegraphics{figures/diagrams/coordinate_chart.pdf} \caption{A coordinate chart of the sphere, \(\c{S}_2\).
	}
\end{figure}
...
The triple \((\Gamma(TX), \oplus, \odot)\label{def:tanget_bundle_module}\) then satisfies the conditions of a \emph{vector space} with respect to this \emph{ring}.
...
An algebra whose prodcut is a Lie bracket is termed a \emphmarginnote{Lie algebra}.
...
.. \partial \phi_n\) produce a \emph{smooth local frame} called a \emphmarginnote{coordinate frame\label{def:coordinate_frame}}.
A \emphmarginnote{smooth local frame\label{def:smooth_local_frame}} a set of smooth (restricted to an open set \(U\)) vector fields that at each point span the tangent space.
...
The lack of existence of a global basis for \(\Gamma(TX)\) innitially makes it difficult to see how to parametrise smooth vector fields on manifolds in a computer, as patching together local chart while maintaining smoothness without a basis is tricky.

The failure of the the existence of a smooth global basis is exactly due to \(\Gamma(TX)\) being a module rather than a vector space.
...
\begin{definition}[Generator of a module]
	...
	A \emph{generator} of the module is a subset \(G \subset M\) such that for any element \(m \in M\) there exsits coefficients \((r_g)_{g \in G}, \; r_g \in R\) such that 
	\[
		m = \sum_{g\in G} r_g g	
	.\]
	If there exists a generator of \(M\) that is finite we say that it is \emphmarginnote{finitely generated}
\end{definition}
...
\begin{proof}
	...
	We can use these charts to create local bases for the tangent bundle, and a \emphhyperref{partition of unity}{} subbordinte to the trivialisation to smoothly glue together local vector fields represented in these local bases into a global vector field.
\end{proof}
...
\end{enumerate}
Using this result therefore, we will always be able to pick a finite set of fields from which we can paremetrise the space of vector fields.
...
We say that a vector field is \(V\in \Gamma\del{TX}\) \emphmarginnote{tangent to a submanifold} if this projection leave the vector field unchanged, and that is it \emphmarginnote{normal to a submanifold} if the vector field is zero under this projection.

For manifolds with boundary, \(X\), we say that an \emphmarginnote{inward-pointing vector field\label{def:inward_pointing}} is a vector field \(V\in \Gamma(TX)\) if at every point on its restriction to the boundary it is normal to the boundary and the tanget vectors point \emph{into} the interior. We define the converse for \emphmarginnote{outward-pointing vector fields\label{def:outward_pointing}}.
...
Fortunatly however, this derivative is exactly given by the \emph{Lie bracket}, giving this a geometric interpretation.
...
\end{theorem}
This gives the Lie derivative therefore all the structute of a \emph{Lie algebra}.
...
There do exist Lie group that are subgroups of \(\groupGL[v, \C]\), but we ill not consider these.
...
The resulting \emph{quotient space}, \(X = G/H\) is called a \emphmarginnote{homogenous space}. The action of \(G\) on \(X\), \(\cdot: G \x X \to X\) is defined by mapping the original group action to the quotient space. For a point \(x\in X\) and a representative element \(g_x \in G\), \(g \cdot x = g g_x H = g_y H = y\). This action is independant of the choice of \(g_x\) made, and so is well-defined. This action is a  \emphmarginnote{transitive action} as using it the group \(G\) maps every point in \(X\) to every other point in \(X\).

A common example of this is that the \(n\)-sphere is a homogenous space of \(\groupO\), \(\c{S}_n \sim \groupO{n+1} / \groupO\).
...
If there is only one orbit, the action is transitive and the space a homegenous space.
Such a symmetry partitions the manifold into a set of \emphmarginnote{orbits}, \[ G\cdot x = \cbrcolon{g\cdot x}{g\in G}.\]
For each point, we can define the \emphmarginnote{stabaliser} as \[G_x = \cbrcolon{g\cdot x = x}{g\in G},\] the set of elements that leave \(x\) in place.
An action is said to be a \emphmarginnote{free action} if the stabaliser of each point is the identity element only, i.e. that no non-identity group element leaves a point stationary.
...
If for all \(h\in H\) \(\theta_h: N\to N\) is an isomorphism on \(N\), the we can define a joint group action of \(H\) and \(N\) together by
\[
...
We can extend the analysis of groups in general by considering \emphmarginnote{(linear) representations} of a group, which is any \emph{group homeoporphism} \(\rho: G\to \groupGL[n]\), i.e. that
...
A \emphmarginnote{faithful} representaiton is one that is an injection into \(\groupGL\).
...
% % Previously we saw how we can define a \emphhyperref{frame}{def:coordinate_frame} as the basis of a tangent space, and that under changes in charts the coefficients of a tanget vector expressed in those charts can be related by an element of \(\groupGL\), a \emphhyperref{change of coordinate}{def:change of coordinates}.
...
% % This implies that the \emphmarginnote{space of frames at a point}, \(F_p\), is homeomorphic to \(\groupGL\), but is not itself a group as we have no prefered "identity" frame.
...
% % It is very rare that smooth sections of the frame bundle exist, and is equivalent to the tangent bundle being \emphhyperref{parallisable}{}.
...
The metric natually extends to a map on vector fields, 
\[
...
From the definition of a Riemannian metric, we can then define the \emphmarginnote{length} of a tanget vector as \[\norm{v}_g = \innerprod{v}{v}_g^{1/2} = g_p(v, v)^{1/2},\] and the \emphmarginnote{angle between tangent vectors} as \[\cos \theta = \frac{\innerprod{v}{w}}{|v|_g|w|_g}.\] 
...
.\]
This makes the gradient the unique \emph{vector field} (not \emph{covector field}) satifying
\[
...
% In particalr, when the isometry is a map between the same manifold, \(F: X \to X\), this is an \emph{automorphism} of the manifold, a self-isomety.
% We denote this set of isomorphisms % \(\iso(X)\).
% These are the intrinsic isometries of the manifold.
% For example for \emphhyperref{def:homogenous_space}{homogenous spaces} of Lie groups, this will be the original Lie group.
...
If we have a Riemannain manifold \((X, g)\) and a submanifold \(S\subset X\), immersed or embedded, with or without a boundary, then this manifold automatically inherits a metric, \(\inclusion^* g\), the \emphmarginnote{induced Riemannain metric}, where \(\inclusion: S\to X\) is the inclusion map.
...
A cornerstone result of Riemannain geometry says that we can do one step better with Riemannian manifolds, and preserve the metric as well.
\begin{theorem}[Nash embedding theorem]
	Let \((X, g)\) be an \(n\)-dimension Riemannain manifold.
...
We can then (with care) use mathematical tools that apply to Eucldiean space and use them for analysis on manifolds.

On the practical side, this gives us a good way to represent Riemaninan manifolds in a computer.
...
A \emphmarginnote{regular curve} is a curve for which the velocity \(\gamma'(t) \neq 0\) everywhere, so that it has no "kinks".
...
We the define the \emphmarginnote{length of a curve} for any admissible curve \(\gamma: \sbr{a, b}\to X\)  as
\[L_g\del{\gamma} = \int_a^b \norm{\gamma'(t)}_g\d t.\]
This definition obeys a few natural properties; it is additive if we split a curve into sections, it is independant of any reparametrisation of \(\sbr{a,b}\), and it is invariant under isometries.

We define the \emphmarginnote{speed of a curve} as \(\norm{\gamma'(t)}_g\), and a \emphmarginnote{unit-speed curve} as one such that its speed is one everywhere.
...
Using this, we can define the \emphmarginnote{Riemannian distance function} as
\[
...
While this may seem odd, it turns out that differntial forms are exactly the type of objects to which we can apply a coherent theory of integration to on smooth manifolds, and also differentiation, but in a different sense to that of Lie derivatives.
This notion is called the \emph{exterior derivative} and it is a genrialisation of the notion of the gradient of a smooth function.
This sense of derivative generalises the notions of divergence, cross products and curl.

In \cref{sec:alternating_tensors} we saw two ways of mapping alternating tensors to alternating tensors of differnet degrees, the \emph{wedge product} and the \emph{interior product}.
Both generalise immediatly to differntial forms by applying them pointwise.
...
We can express a differntial form in a chart via basis functions.
...
remebering that \(\bb{I}\) is the set of strictly increasing multiindexes, and \(\d\phi^I = \d\phi^{i_i}\wedge ... \wedge\d\phi^{i_k}\), and \(\omega_I\) are smooth functions.
...
Exterior derivatives are a type of differnetial operator that we can apply to \(k\)-forms to turn then into \(k+1\)-forms.
...
More generally, we can build off of this to extend the differnetial to higher forms.
...
\]
Where \(\d\omega^I\) is the differntial of the \(I\) coordinate function, and the second line is its expansion into basis covector fields.
...
The existence of an orientation of a smooth manifold however is far from guaranteed, to the extend that we calssify manifolds as orientable or non-orientable.
An \emphmarginnote{orientable manifold\label{def:orientable_manifold}} is a manifold for which an orientation exsits.
An \emphmarginnote{oriented manifold} is a smooth manifold, along with a particualr choice of orientation.
...
\begin{definition}[Volume form]
	...
	A \emph{volume form} \(\omega\in \Omega^n(X)\) is a \emph{non-vanising} \(n\)-form on \(X\).
\end{definition}
The \emph{non-vanising} part of this definition is key, it means our volume form is not allowed to assign zero volume to any point on the manifold.
...
Note we can flip which maps preserve and reverse orientation reversing the orientation on one of \(X\) ands \(Y\).
...
\begin{definition}[Induced orientation]
	...
	Then the \((n-1)\)-form \(\inclusion_{\partial X}^* (N \iprod \omega)\) is a volume form on \(\partial X\), and induces the same orientation for any choice of \(N\), where \(\inclusion_{\partial X}^*: \Gamma(TX)\to \Gamma(T\partial X)\) is the pullback of a differntial form from \(X\) to \(\partial X\).
\end{definition}
...
On Riemannain manifolds, we are always able to pick smooth, \emph{orthonormal} local frames on the manifold.
...
Our choice of notation \(\d\vol_g\) will be motivated shortly, but it does \emph{not} mean that the Riemannain volume form is the exterior derivative of an \((n-1)\)-form.
...
\begin{enumerate}
	...
	\]
	\item Positive volume forms have \emph{positive integral}. If \(\omega\) is a positivly oriented volume form, then \(\int_X \omega > 0\).
...
We use this correspondence to define the \emphmarginnote{divergence on a Riemannain manifold\label{def:riemannain_divergence}} \(\dive: \Gamma(TX) \to C^\infty(X)\) by
\[
...
\begin{theorem}[Divergence theorem]
	Let \((X, g)\) be an oriented Riemannain manifold with boundary, and \(V\in\Gamma(TX)\) with compact support.
	Then
	\[
...
\]
and also via the \emphhyperref{Levi-Civita connection}{}.
...
In order to work with measure theory on manifolds (\cref{sec:measure_theory}), and therefore deal with probability formally, we need to define a base measure on a Riemannian manifolds.
We can construct this on a Riemannian manifold using the Reisz representaiton theorem and the integration defined by the volume form.

\begin{theorem}[Riesz-Markov-Kakutani representaiton theorem]
	Let \(X\) be a locally compact Haussdorff space, and let \(\Lambda\) be a positive linear functional on \(C(X)\). Then there exists a \(\sigma\)-algebra, \(\sigalg{X}\), on \(X\) continaing all open sets on \(X\), and there exists a unique positive measure \(\mu\) on \(\sigalg{X}\) which represents \(\Lambda\) in the sense that:
	\begin{enumerate}
...
\begin{definition}
	...
	satisfying
	\begin{enumerate}
		\item \(\nabla\) is \emph{linear} in the first argument over \emphhyperref{the module \((\Gamma(TX), \oplus, \odot)\)}{def:tanget_bundle_module},
...
For two vector fields \(V, W\in \Gamma(TX)\), using the properties of the connection, then we have in the chart that\marginnote{Coordinate expression of a connection}
...
The most important application of connections is that they tell us how to transport tensors, and in particalr vectors, along a path on a manifold in a coherent way.
To do so, let us consider a vector field on a curve.

Let \(\gamma: I \to X\) be a smooth curve on the manifold, and let a \emphmarginnote{smooth vector field on a curve} be a smooth map \(V: I\to TX\) such that \(V(t) \in T_{\gamma(t)}X\). Such a vector field is said to be \emphmarginnote{extendable} if there exists a smooth vector field \(W \in \Gamma(TX)\) such that \(V(t) = W_{\gamma(t)}\). This definition is necessary as if our path crosses itself, we may assign two different vectors to the same location.
We say that an \emphmarginnote{extentsion of an extendable vector field on a curve} is any vector field \(\tilde V\) such that \(V(t) = \tilde V_{\gamma(t)}\). 

We can similarly define \emphmarginnote{extendable smooth tensor fields on a curve}.
...
\begin{definition}[Covariant derivative along a curve]
	...
	Let \(\tilde V\) be an extentsion of \(V\).
	The the \emph{covaraint derivative of \(V\) on \(\gamma\)} is given by
	\[
...
The \emphmarginnote{Hopf-Rinow theorem} is a cornerstone result that states for any connected Riemannian manifold, geodesic completeness and completeness in the metric sense (i.e. that all Cauchy sequences converge) are equivalent.
...
The existence of a flat connection on a smooth manifold is equivalent to the tangent bundle being \emphhyperref{parallisable}{}, as we can define non-vanishing coordinate fields on the manifold by parallel transporting a frame at a given point around the manifold. 
...
\begin{enumerate}
	...
	\[\Gamma_{ki}^l g_{lj} + \Gamma_{kj}^lg_{il} = \partial\phi_i(g_{ij}) .\]
	\item Parallel transport of a vector along a curve preserves their Riemannian norm.
	\item Parallel transport of a pair of vectors along a curve preserves their Riemannain inner product.
	\item Parallel transport of an orthonormal frame along a curve remains orthonormal.
\end{enumerate}
Unfortunatly, this does not uniquely specify a connection.
...
\end{theorem}
We call this unique connection the \emphmarginnote{Levi-Civita conncection\label{def:levi_civita_connection}}, and when ambiguous we denote it \(\nabla_g\), however when we refer to a connection without specifying it, we always refer to the Levi-Civita connection as it is the canonical choice on a manifold.
In addition, when we refer \emphhyperref{geodesics}{def:geodesic}, the \emphhyperref{exponential}{def:exponential_map} and \emphhyperref{logarithm}{def:logarithm_map} maps and \emphhyperref{parallel transport}{def:parallel_transport} on a Riemannain manifold, they are with respect to the Levi-Civita connection unless otherwise specified.

\subsection{The Laplace-Beltrami operator}
One particualar operator that is of great use and is well studied is the \emphmarginnote{Laplace-Beltrami operator}.
...
.\]
On Riemannain manifolds we can define this in two ways,
\[
	\Delta: C^\infty(X) \to C^\infty(X) \quad \quad \Delta f  = \dive(\nabla f) = \tr\del{\nabla^2 f}
,\]
