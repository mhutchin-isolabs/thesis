...
\emphmarginnote{Score-based generative models} also called diffusion models \parencite{song2019generative,song2020score,ho2020denoising,nichol2021beatgans} formulate generative modelling as a denoising process.
...
The objective is to incorporate the geometry of the problem at hand by respecting the natural setting of the data modelled.
...
More specifically our setting will be a \emphhyperref{complete}{def:geodesically_complete}, \emphhyperref{orientable}{def:orientable_manifold}, \emphhyperref{connected}{def:connected_space}, \emphhyperref{boundaryless}{def:manifold_with_boundary} \emphhyperref{Riemannian}{def:riemannain_manifold} manifold, \((\c{M}, g)\).
...
If we assume that a density of a measure exists at any point, we assume that it exists relative to the \emphhyperref{Riemannain volume measure}{def:riemannain_volume_measure}, \(\d\vol_g\), the measure induced by the Riemannain volume form on the manifold.
...
\begin{figure}[t]
	...
	\begin{subfigure}[t]{\textwidth}
		\includegraphics[width=.66\textwidth]{images/rsgm/so3_conditional.pdf}
