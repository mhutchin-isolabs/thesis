...
We can also target the \emphmarginnote{Riemannian normal} \parencite{pennec2006Intrinsic} distribution.
We active this by setting \[U(x) = d_\c{M}(x, \mu)^2/(2 \gamma^2)\], where $d_\c{M}$ is the geodesic distance and $\mu \in \c{M}$ is an arbitrary mean location.
...
Finally, in the case where the manifold is a compact manifold we can make a particularly useful choice.
We can choose \(U(x) = 0\), giving simple Brownian motion, \[ \d \v{X}_t = \d\v{B}_t^M\].
On a compact manifold, this will converge to a density proportional to the Riemannian volume form, simply the \emphmarginnote{uniform distribution on a manifold} given by \(\mu = \frac{\vol_g}{\int_\c{M} \vol_g} \).
Regardless of the starting distribution the SDE will converge to the uniform distribution exponentially quickly.
...
\end{theorem}
\begin{proof}
	The proof is identical to that of \cref{thm:fokker-plank_time_reversal}, except we replace the various differential operators by their manifold generalisations, in particular using the identity that \[\Delta_g p_t = \dive(\nabla_g p_t).\]
\end{proof}

We also provide generalisation of stronger the in the Euclidean case, e.g. see \textcite[Theorem 4.9]{cattiaux2021time}.
...
The stronger results tell us that the distribution on the whole space of \emph{paths} of the SDE match.
