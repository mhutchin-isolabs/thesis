...
The proof of the manifold implicit score matching loss, drawing links between the denoising score matching loss and the implicit score matching loss, is presented \cref{sec:manifold_ism_proof}.
...
The distance \(d_\c{M}(x,y)\) is defined as the infimum of the length of all the curves on \(\c{M}\) joining \(x\) and \(y\).
...
Throughout this section \(\c{M}\) is a \(d\)-dimensional smooth manifold, \(\TM\) its tangent bundle and \(\TM^*\) its cotangent bundle.
...
\end{equation}
A curve \(\gamma\) on \(\c{M}\) is said to be a geodesic if \(\dot{\gamma}\) is parallel to \(\gamma\).
...
\paragraph{Stratonovich integral}
For reasons that will become clear in the next paragraph, it is easier to define Stochastic Differential Equations (SDEs) on manifolds with respect to the Stratonovich integral \cite[Part II, Chapter 3]{kloeden:platen:2011}.
...
Let \((\v{X}_t)_{t \geq 0}\) and \((\v{Y}_t)_{t \geq 0}\) be two real continuous semimartingales, then we define the Stratonovich integral as follows for any \(t \geq 0\) \begin{equation} { \int_0^t \v{X}_s \circ \mathrm{d} \v{Y}_s = \int_0^t \v{X}_s \mathrm{d} \v{Y}_s + \tfrac{1}{2} [\v{X}, \v{Y}]_t .
	}
\end{equation}
...
We refer to \textcite{kurtz1995stratonovich} for more details on Stratonovich integrals.
...
In particular, Stratonovich calculus satisfies the ordinary chain rule making it a useful tool in differential geometry which makes a heavy use of diffeomorphism.
Finally, we have the following correspondence between Stratonovich and It\^o SDEs.
...
A continuous \(\c{M}\)-valued stochastic process \((\v{X}_t)_{t \geq 0}\) is called an \(\c{M}\)-valued semimartingale if for any \(f \in \mathrm{C}^\infty(\c{M})\) we have that \((f(\v{X}_t))_{t \geq 0}\) is a real valued semimartingale.
...
We denote \(p_{\textup{ref}}^m\) the Hausdorff measure of the manifold (which coincides with the measure associated with the Riemannian volume form \parencite[see][Theorem 2.10.10]{federer2014geometric} and \(p_{\textup{ref}} = p_{\textup{ref}}^m / p_{\textup{ref}}(\c{M})\) the associated probability measure.
...
\begin{definition}[Brownian motion]
	Let \((\v{b}_t^\c{M})_{t \geq 0}\) be an \(\c{M}\)-valued semimartingale.
...
\begin{proposition}[Intrinsic view of Brownian motion]
	...
	Let \((\v{b}_t^\c{M})_{t \geq 0}\) be an \(\c{M}\)-valued semimartingales.
...
In particular the previous proposition provides us with an \emph{intrinsic} way to sample the Brownian motion on \(\c{M}\) with initial condition \(\v{b}_0^\c{M}\).
...
\end{equation}
By definition of the Stratonovich integral we have that for any \(k \in \{1, \dots, d\}\) \begin{equation} { \mathrm{d} \v{X}_t^k = \sum_{j=1}^d \{ \v{E}_t^{k,j} \mathrm{d} \v{b}_t^k +\tfrac{1}{2} \mathrm{d} [\v{E}_t^{k,j}, \v{b}_t^j]_t \} .
	}
\end{equation}
...
In this section, we summarize results from \textcite{jorgensen1975central} establishing the limiting behaviour of Geodesic Random Walks (GRWs) when the step size of the random walk goes to \(0\).
...
Again, we emphasize that in order to make the proof completely rigorous one needs to derive regularity properties of \(v\).
...
To do so, we will verify that under a finite entropy assumption, the conditions \(u \in \rm{dom}(\c{A}_{R(\bb{P})})\) and \(\c{A}_{R(\bb{P})}u \in \mathrm{L}^1(\bb{P})\) are fulfilled for a class of regular functions \(u\).
...
Standard Euclidean SGMs rely on a Ornstein--Uhlenbeck (OU) forward process which can easily be simulated since \(\v{X}_t | \v{X}_0\) is Gaussian.
