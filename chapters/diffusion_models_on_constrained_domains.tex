...
In the previous chapter we extended the framework of score-based generative modelling to a wide range of Riemannian manifolds, enabling the specification of inherent structural properties of the modelled domain.
...
Such constrained manifolds comprise a wide variety of settings---including polytopes and convex sets of Euclidean spaces---and are studied across many disciplines, ranging from computational statistics \parencite{Morris2002}, over robotics \parencite{han2006inverse} and quantum physics \parencite{lukens2020practical}, to computational biology \parencite{Thiele2013}.
...
However, the introduced samplers suffer from the same limitations as the log-barrier and reflected Brownian motion approaches we discuss for more complex manifolds and constraints beyond the hypercube setting.
...
A core theoretical contribution is to show that this new discretisation converges to the reflected stochastic differential equation by using the invariance principle for stochastic differential equations with boundary \parencite{stroock1971diffusion}.
...
\begin{figure}[t]
	...
	\caption{An example of a polytope in two dimensions, with example geodesics under the usual Euclidean metric, the log-barrier metric, and boundary reflection.}
	\label{fig:polytope}
\end{figure}
...
We consider throughout this section that the constraints we encounter form a compact set on the manifold.
...
Among these functions, the \emph{log barrier} is the most popular among practitioners ~\parencite{lee2017Geodesic}.
...
In general this is a highly non-trivial optimisation problem, however.

In special cases we can exploit the constraints at hand to compute this distance function.
For a convex polytope \(\c{M}\) defined by the constraints \(\mathrm{A} x < b\), with \(\mathrm{A} \in \R^{m \times d}\) and \(b \in \R^m\), the log barrier \(\phi: \ \c{M} \to \R_+\) is given for any \(x \in \c{M}\) by \begin{equation} \label{eq:log_barrier_polytope} { \phi(x) = - \sum_{i=1}^m \log(\langle \mathrm{A}_i, x\rangle-b_i).
	}
\end{equation}
...
Briefly we introduce the technical detail needed to define a reflected Brownian motion.
...
See \Cref{sec:refl-brown-moti} for a rigorous introduction.`'

We say that \((\bar{ \v{B}}_t, \v{k}_t)_{t \geq 0}\) is a solution to the \emph{Skorokhod problem} \parencite{skorokhod1961stochastic} if \(( \v{k}_t)_{t \geq 0}\) and \((\bar{ \v{B}}_t)_{t \geq 0}\) are two processes satisfying mild conditions, such that for any \(t \geq 0\), \begin{equation} \label{eq:rbm} \bar{ \v{B}}_t = \bar{ \v{B}}_0 + \v{B}_t - \v{k}_t \in \c{M}, \end{equation} and \[{\abs{ \v{k}}_t = \int_0^t \mathbf{1}_{\bar{ \v{B}}_s \in \partial \c{M}} \mathrm{d} \abs{\v{k}}_s}, \quad { \v{k}_t = \int_0^t \v{n}(\bar{ \v{B}}_s) \mathrm{d} \abs{\v{k}}_s ,}\] where \((\abs{ \v{k}}_t)_{t \geq 0}\) is the total variation of \(( \v{k}_t)_{t \geq 0}\) and \( \v{n}\) is the \emphhyperref{outward normal}{def:outward_pointing} to \(\c{M}\).
When \((\bar{ \v{B}}_t)_{t \geq 0}\) hits the boundary, the condition \[{ \v{k}_t = \int_0^t \v{n}(\bar{ \v{B}}_s) \mathrm{d} \abs{\v{k}}_s ,}\] tells us that \(- \v{k}_t\) "compensates" for \({ \v{B}}_t\) by pushing the process back into \(\c{M}\) along the inward normal \(- \v{n}\).
Meanwhile, the condition \[{\abs{\v{k}}_t = \int_0^t \mathbf{1}_{\bar{ \v{B}}_s \in \partial \c{M}} \mathrm{d} \abs{\v{k}}_s}\] can be interpreted as \( \v{k}_t\) being constant when \((\bar{ \v{B}}_t)_{t \geq 0}\) does not hit the boundary.
...
\begin{proposition}[Reflected Brownian motion Fokker-Plank equation]
	For any \(t > 0\), the distribution of \(\bar{ \v{B}}_t\) admits a density with respect to.\
...
\begin{theorem}[Time reversal of reflected Brownian motion]
	...
	There exist \((\v{k}_t^\vsla{})_{t \geq 0}\) a bounded variation process and a Brownian motion \(( \v{B}_t)_{t \geq0}\) such that \begin{equation} { \v{X}_t^\vsla{} = \v{X}_0^\vsla{} + \v{B}_t - \v{k}_t^\vsla{} + \int_0^t \nabla \log p_{T-s}(\v{X}_s^\vsla{}) \mathrm{d} s .
		}
	\end{equation}
...
In the case of a reflected Brownian motion, it is possible to compute an equivalent ordinary differential equation in order to perform likelihood evaluation.
...
\begin{algorithm}[t]
	\caption{\emph{Reflected Random Walk}.
		Discretisation of the stochastic differential equation \(\mathrm{d} \v{X}_t = b(t, \v{X}_t) \mathrm{d} t + \mathrm{d} \v{B}_t - \mathrm{d} \v{k}_t\).
	}
	\label{alg:rrw}
	\begin{algorithmic}
...
Building on the reflected diffusion model introduced in \cref{sec:reflected_diffusion_model}, we now propose a new discretisation of the reflected Brownian motion that is much cheaper to compute than the typical one previously discussed.

As a reminder, the usual discretisation of the reflected stochastic differential equation is to \begin{enumerate*}[label=(\roman*)] \item consider a classical step of the Euler-Maruyama discretisation (or the Geodesic Random Walk in the Riemannian setting) and \item reflect this step according to the boundary defined by \(\partial \c{M}\).
\end{enumerate*}
...
The sampler we propose is similar to a classical Euler-Maruyama discretisation of the Brownian motion, except that, whenever a step would carry the Brownian motion outside the constrained region, we reject it and move on to the next step.
...
\begin{proof}
	...
	In the Euclidean setting, this result, accompanied by mild regularity and growth assumptions, ensures that the discretization weakly converges to the original stochastic differential equation.
...
% \textcite{de2022riemannian} extended the work of \textcite{song2020score} to
...
% Existing applications of denoising diffusion models on Riemannian manifolds have been focused on well-known manifolds for which one can find metrics so that the framework of \textcite{de2022riemannian} applies.
...
% generation. \textcite{corso2023diffdock} use diffusion models on
...
\subsection{Approximation schemes of reflected stochastic differential equations}
...
We refer to \cite{pilipenko2014introduction} for an introduction to the discretisation of reflected stochastic differential equations.
...
To learn a distribution over this space, we leverage the methodology introduced in \cref{sec:constrained_diffusion_models} for the polytope component \(\mathbb{P}^3\) and in \cref{cpt:riemannian_score_based_models} for the torus component \(\mathbb{T}^4\).
...
\begin{table*}
	\begin{tabular}{lccccc}
		...
		\scshape \textcite{liu2024mirror}            & \cmark                                                                                        & \cmark                                   & \(\text{\small convex}\;{\subset}\;\R^d\)              & \xmark \\
