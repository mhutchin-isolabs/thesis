...
It also induces the Laplace-Beltrami operator $\Delta$ and consequently a Brownian motion with density (w.r.t. the volume form \footnote{We assume that $\c{M}$ is orientable and therefore that a volume form exists })whose density is given by the heat equation ${\partial_t } p_t = \Delta p_t$.
...
Under similar conditions, $ \v{X}_t$ admits a density $p_t$ w.r.t.\ the Lebesgue measure and we have that $\partial_t p_t = \tfrac{1}{2} \mathrm{Tr}(\mathfrak{g}^{-1} \nabla^2p_t)$.
...
A corollary is that, for $\gamma > 0$ small enough and for any $k \in \N$, the probability that $X_{k+1}^\gamma = X_k^\gamma$ is upper bounded \emph{uniformly} w.r.t. $X_k^\gamma \in \c{M}$.
