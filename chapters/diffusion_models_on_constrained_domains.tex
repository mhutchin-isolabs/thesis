...
However, the introduced samplers suffer from the same limitations as the log-barrier and reflected Brownian motion approches we disucess for more complex manifolds and constraints.
...
The estimation of the score term \(\nabla \log p_t\) is done by minimising the \(\mathrm{ism}\) loss function (\cref{prop:manifold_ism}) .
...
Instead, we rely on an approximation, using a \emph{retraction} (see \textcite{absil2012projection, boumal2023introduction} for a definition and alternative schemes). \todo{state th eretraction}
...
Another approach to deal with the boundary geometry of \(\c{M}\) is to use the standard metric \(\mathfrak{h}\) and forward dynamics of \(\c{N}\) and constraining it to \(\c{M}\) by \emph{reflecting} the process whenever it would encounter a boundary.
...
In this section, we prove that the proposed Metropolis sampling-based process (\cref{alg:metropolis_approx}) corresponds to a valid discretization of the reflected process, \cref{def}\todo{fill ref}, justifying its use in diffusion models.
...
\end{definition}
We call this process the \emph{rejection approximation of RBM} since in practice, \(Z_k^\gamma\) can be sampled using rejection sampling, see~\Cref{alg:rejection_sampling}.
\begin{algorithm}[H]
	...
	\end{algorithmic}
\end{algorithm}
The key between the rejection and 
...
We train the models on simple bimodal synthetic data on the hypercube and the simplex, and compare the results visually in \cref{fig:synthetic_hypercube,fig:synthetic_dirichlet} and using the MMD distance \parencite{gretton2012kernel} between the data distribution and the learnt distribution in \cref{tab:synthetic_polytope_mmd}.
...
\begin{figure*}[t]
	...
	\begin{subfigure}{.195\textwidth}
		...
		\label{fig:app:synthetic_hypercube_log-barrier}
	\end{subfigure}
	\hfill
	\begin{subfigure}{.195\textwidth}
		\centering
...
\begin{figure*}[t]
	\centering
	\begin{subfigure}{0.195\columnwidth}
		\includegraphics[width=\linewidth]{images/constrained_diffusion/spd_l_data.png}
		\caption{Data distribution}
	\end{subfigure}
	\hfill
	\begin{subfigure}{0.195\columnwidth}
		\includegraphics[width=\linewidth]{images/constrained_diffusion/spd_l_logbarrier.png}
		\caption{Log-barrier model}
	\end{subfigure}
	\hfill
	\begin{subfigure}{0.195\columnwidth}
		\includegraphics[width=\linewidth]{images/constraind_diffusion_rejection/spd_main_reflected.png}
		\caption{Reflected model}
	\end{subfigure}
	\hfill
	\begin{subfigure}{0.195\columnwidth}
		\includegraphics[width=\linewidth]{images/constraind_diffusion_rejection/spd_main_rejection.png}
		\caption{Reflected model}
	\end{subfigure}
	\hfill
	\begin{subfigure}{0.195\columnwidth}
		\includegraphics[width=\linewidth]{images/constrained_diffusion/spd_l_uniform.png}
		\caption{Uniform}
	\end{subfigure}
