...
We can then define the \emphmarginnote{wedge product} of \(F\in \Lambda^k(V^*), G\in \Lambda^l(V^*)\) as \[ F\wedge G = \frac{(k+l)!
...
For covectors \(\omega^1, .
.. \omega^k\), and vectors \(V-1, ..., v_k\), \[\omega^1 \wedge ... \wedge \omega^k(v_1, ..., v_k) = \det \del{\omega^i(v_j)}\] where \(\det\) is the usual matrix determinant.
...
It allows us to discuss the proximity of points without needing to being able to say how far apart exactly they are.
...
A smooth manifold is a topological manifold with additional structure.
In this case, we want to ensure that the charts and transition maps of the atlas of the manifold as \emph{smooth}.
For a chart \((U, \phi)\) to be smooth we require that \(\phi: U \to \R^k\) be smooth (\(\phi\in C^\infty\)), if all its component functions have continuous differentials of all orders.
An atlas is said to be \emphmarginnote{smoothly compatible} is for every pair of charts \((U, \phi), (V, \psi)\) the transition maps \(\phi \circ \psi^{-1}\) and \(\psi \circ \phi^{-1}\) restricted to \(\phi(U\^V)\) and \(\psi(U\^V)\) are smooth.
...
Bearing this in mind, we define a \emphmarginnote{derivation at \(p\)} as a linear function \(w: C^\infty(\R^n)\to \R\) that follows the product rule \[\sbr{w(fg)}(p) = f(p) w(g) + g(p) w(f)\].
...
This then give us a natural way to define the \emphmarginnote{tangent space at a point on a manifold}, \(T_pX\), as the vector space of derivations of \(C^\infty(X)\) at \(p\).
For a \(n\)-dimension manifold is \(n\)-dimension vector space, isomorphic (in the vector space sense) to \(\R^n\).
...
\[\sbr{\d F_p (v)}\del{f} = v \del{f \circ F}, \quad v\in T_pM, \; f\in C^\infty(Y) \].
...
We can also study how the differential behaves in local coordinates.
...
Applying the definition of the differential and the chain rule to a function \(f\in C^\infty (\R^m)\), we get \[\d F_p\del{\eval{\pd{}{x_i}}_p}f = \eval{\pd{}{x_i}}_p \del{f \circ F} = \sum_{j=1}^m \pd{f}{y_j}\del{F(p)} \pd{F_j}{x_i}(p) = \del{\sum_{j=1}^m \pd{F_j}{x_i}(p) \eval{\pd{}{y_j}}_{F(p)}} f\].
...
If we have \(p\) in two charts \((U, \phi)\), \((V, \psi)\) and a vector at \(p\), \(v = \sum_{i=1}^n a_i \eval{\pd{}{\phi_i}}_p = \sum_{i=1}^n b_i \eval{\pd{}{\psi_i}}_p\), then the basis coefficients are related by \(b_j = \sum_{i=1}^n \pd{\psi \circ \phi^{-1}_j}{x_i} a_i\).
...
\begin{proposition}[The tangent bundle is a smooth manifold]
	...
	With respect to this structure, the projection \(\pi: TX \to X\) is smooth.z \end{proposition} 
...
This makes a vector field a function \(V: C^\infty(X) \to C^\infty(X)\), and is a \emphmarginnote{derivation of \(C^\infty(X)\)} (they satisfy the product rule).
All derivations of \(C^\infty(X)\) are vector fields.
...
For \(U,V,W \in \Gamma{TX}\) we have \begin{enumerate}[noitemsep] \item \(\sbr{V,W}\) is bilinear.
...
In essence these assign a to each point in the base space a point in its fibre, its preimage under the projection, \(\pi^{-1}(p) = F_p\).
The space of sections are denoted \(\Gamma(E)\).
...
\emphmarginnote{Vector bundles} are fibre bundles where the typical fibre is isomorphic to \(\R^n\).
...
These are not the interesting cases for stochastic process however.
...
This index set is both a metric space, and so has a topology, and is also \emph{totally ordered}.

By fixing the index argument this can be thought of as an indexed collection of random variables \(\cbrcolon{X_i: \Omega \to S = X(\cdot, i)}{i\in I}\).
...
The choice of forward noising SDE can have a significant impact on performance of a diffusion model. Interesting empircal studies can be seen for example in \textcite{karras2022Elucidating} \todo{more cites?}.
...
The next step in producing a diffusion model is learning the score term that appears in the reverse SDE, \(\nabla_{\v{X}_t} \log \pi_t(\v{X}_t)\).
At first glance this may seem difficult, but a strong literature exists on learning this term, known as the score matching.
...
In practise this integral is not solvable, so we must approximate it.
Typically, we use standard Monte Carlo approximation, approximating the integral as a sum over samples from \(\pi_t\).
To do this we can utilise the fact that \[\pi_t(\v{X}_t) = \int p_{t|0}(\v{X}_t) \pi_0(\v{X}_0) \d \v{X}_0\].
We can sample from this as we have as our dataset samples from \(\pi_0 = \pi\), and we have access to the transition density \( p_{t|0}\) from the SDEs we used as our forward noising SDE.

One other feature of note in this objective is the computation of the divergence of the score.
While this is achievable automatically using automatic differentiation packages, it is expensive in high dimension, scaling in cost with \(\c{O}(d^2)\).
It is possible to apply to this the Hutchinson trace trick estimator \parencite{hutchinson1989stochastic} to return to get a loss function that scales with \(\c{O}(d)\) at the cost of higher variance, as detailed in \textcite{song2020sliced}.
In practise this additional variance does not seem to be too important, as the variance of the Monte Carlo estimation of the integral over \(\pi_t\) is larger.

This objective matches the score at time \(t\) only. 
In order to match the score at all times, we integrate this objective over all times \(t \in \sbr{0, T}\), so that 
\[\ismloss{}(\scorenet{}) = \int_{\sbr{0, T}} \lambda(t) \ismlosst{}(\scorenett{}) \d t\].
Notice the weighting function applied over all \(t\).
While this does not affect the minima of the objective, it does rebalance where a network will prioritise learning the score most.
Manty choices for \(\lambda(t)\) exist ... \todo{Add some}.
In particular picking \(\lambda(t) = \sigma(t)^2\) can be shown to be equivalent to optimising a maximum likelihood objection on the dataset \cite{huang2021variational,durkan2021maximum}.

\emphmarginnote{Denoising score matching} (DSM)