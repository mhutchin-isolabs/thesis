...
% It also induces the Laplace-Beltrami operator \(\Delta\) and consequently a Brownian motion with density (with respect to.
...
\begin{algorithm}[t]
	\begin{algorithmic}
		...
		\State \(x' \gets \exp_{g}\del{x, \alpha\v{s}}\) \Comment{Exponnetial map step.}
...
While this scheme eonforces a stonger condition on the score function then Neuman conditions, that it is zero on the boundary, not just tangent to the boundary, we find in practise that this works well, and is cheaper than enforcing Neuman conditions only.
