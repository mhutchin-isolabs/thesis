...
We consider the constraints in the way set out as it allows us to consider a slightly larger setting of constraints.
...
While the two methods we introduced in this chapter can be applied to arbitrary constraints, in our applications we focus on two specific settings: \begin{enumerate}[label=(\alph*)] \item \emphmarginnote{polytopes}---\(\c{N} = \R^d\) with linear boundaries forming a convex set, \item \emphmarginnote{symmetric positive definite matrices} (SPD) under maximum trace conditions---\(\c{N}=\mathcal{S}_{++}^{d}\), a quadratic boundary on the elements of the matrix.
\end{enumerate}
...
\begin{figure*}[h]
	\begin{minipage}[b]{0.49\textwidth}
		\centering
		\input{figures/tex/hessian_density.tex}
...
Let us consider a compact set of Euclidean space equipped with a Euclidean metric, a set of constraints, and a Hessian non-Euclidean metric described above.
...
We refer to \cref{sec:score-netw-param} for details on the training and parametrisation of the score function.
...
See \textcite{absil2012projection,boumal2023introduction} for a deeper discussion and alternatives.
...
\begin{enumerate}
	\item \({\abs{ \v{k}}_t = \int_0^t \mathbf{1}_{\bar{ \v{B}}_s \in \partial \c{M}} \mathrm{d} \abs{ \v{k}}_s}\), where \(\v{1}\) is the indicator function, and
...
\end{proof}
Note that contrary to the unconstrained setting, the heat equation has \emphmarginnote{Neumann boundary conditions}, that the change in density on the boundary is tangential to the boundary.
...
\begin{theorem}[Time reversal of Brownian Shorokhod problems]
	...
	\end{equation}
	In addition, for any \(t \in \sbr{0,T}\) we have \begin{equation} \label{eq:skorokhod_reversal} {\abs{\v{k}}_t^\vsla{} = \int_0^t \mathbf{1}_{{ \v{X}}_s^\vsla{} \in \partial \c{M}} \mathrm{d} \abs{\v{k}}_s^\vsla{} , \quad \v{k}_t^\vsla{} = \int_0^t \v{n}({ \v{X}}_s^\vsla{}) \mathrm{d} \abs{\v{k}}_s^\vsla{} .
		}
	\end{equation}
\end{theorem}
\begin{proof}
...
This makes use of a discretised reflection step, detailed in \cref{alg:reflection}.
...
\begin{algorithm}[t]
	...
	\begin{algorithmic}
		...
		\State \(X_{k+1}^\gamma = \text{ReflectedStep}[X_{k}^\gamma, \sqrt{\gamma}
			Z_{k+1}, \{f_i\}_{i \in \mathcal{I}}]\) \Comment{See \cref{alg:reflection}.}
		\EndFor \State { \bfseries return} \(\{ X_k^\gamma\}_{k=0}^{N}\) \end{algorithmic} \end{algorithm} 
\begin{algorithm}[t]
	\begin{algorithmic}
		...
		\State \(\v{s} \gets \v{v} / \norm{\v{v}}_{g}\)
		\While{\(\ell \geq 0\)}
		\State \(d_i = \arg\min_{t\in\R^+} \exp_p(t\v{v}) \in \partial f_i\) \Comment{Boundary intersection.}
		\State \(i \gets \argmin_i\; d_i \; s.t. \; d_i > 0\) \Comment{Select minimum boundary distance.}
		\State \(\alpha \gets \min\del{d_i, \ell} \) \Comment{Minimum distance to boundary or end of transport.}
		\State \(x' \gets \exp_{g}\del{x, \alpha\v{s}}\) \Comment{Exponential map step.}
		\State \(\v{s} \gets \paralleltransport_{g}\del{x, \v{s}, x'} \) \Comment{Transport tangent vector to the boundary.}
		\State \(\v{s} \gets \reflect\del{\v{s}, f_i}\) \Comment{Reflect on the boundary.}
		\State \(\ell \gets \ell - \alpha\) \Comment{Reduce step size by distance travelled.}
		\State \(x \gets x'\)
		\EndWhile
	...
	\caption[]{
		...
		For a discussion of boundary intersection, \(\paralleltransport\), \(\exp_{g}\) and \(\reflect\) please see \cref{def:boundary_intersection,def:manifold_reflection}.
	}
...
\begin{proposition}[Likelihood ordinary differential equation for refelcted Brownian motion]
	...
	Let \((\v{X}_t)_{t \geq 0}\) be given for any \(t \geq 0\) by \[\mathrm{d} \v{X}_t = \tfrac{1}{2} \nabla \log p_t(\v{X}_t) \mathrm{d} t\] and \(\v{X}_0 \sim p_0\), where \(p_t\) denotes the density of \(\bar{\v{B}}_t\) with respect to.
...
Relaxing the assumption that the underlying manifold is a Euclidean space seems possible as the definition of Skorokhod problems extends naturally to the Riemannian manifold setting.
...
For any non-smooth regions of a boundary, for example a corner, we can apply an \(\epsilon-\)smoothing to the corner to make the boundary smooth, resulting in extremely minor practical differences, see \cref{fig:epsilon_smooth}.
...
\begin{proposition}[Implicit score-matching on constrained domains]
	Let \(s \in \mathrm{C}^\infty(\cbr{0,T} \times \R^d, \R^d)\) such that for any \(x \in \partial \c{M}\) and \(t \geq 0\), \(\innerprod{s_t(x)}{\v{n}(x)} = 0\).
...
This result immediately implies we can optimise the score network approximate the score function using the implicit score matching loss function so long as we enforce a Neumann boundary condition, which naturally arises from \cref{thm:reflected_likelihood_ode} and so is a constraint on the score function that does not reduce the modelling capacity of the model.

Following \textcite{liu2022estimating} we can accommodate this boundary condition in our score parametrisation by scaling the score output of the neural network by a monotone function \(h(d(x, \partial \c{M}))\) where \(d\) is the distance from \(x\) to the boundary, where \(h(0) = 0\).
...
While this scheme enforces a stronger condition on the score function then Neumann conditions, that it is zero on the boundary, not just tangent to the boundary, we find in practise that this works well, and is cheaper than enforcing Neumann conditions only.
...
First, we aim to assess the empirical performance of our methods on constrained manifolds of increasing dimension.
...
We present a qualitative comparison of the log barrier and reflected Brownian motion models in \cref{fig:polytope_synthetic}, and observe that both methods recover the data distribution on the two-dimensional hypercube and simplex, although the reflected method produces a better fit.
...
Alternatively, we can parametrise the SPD matrices via their Cholesky decomposition.
...
In summary, the space over which we parametrise the diffusion models is defined as \[\sbr{ \mathrm{L} \in \R^{d(d+1)/2} : \mathrm{L}_{i,i} > 0, \sum_{i,j} \mathrm{L}_{i,j}^2 < C } \times \R^2.\]
...
Quantitatively, the reflected method achieves an MMD of \(0.161_{\pm 0.003}\) with respect to the dataset as opposed to \(0.247_{\pm 0.012}\)d for the log-barrier method.
...
\begin{figure}[h]
	...
	\begin{subfigure}[t]{0.48\textwidth}
		...
		\caption{A commonly-used approximate parametrisation of backbone geometry only considers the \(C_\alpha\) torsion angles \(\Phi\) and \(\Psi\).}
...
\begin{figure}[t]
	...
	\begin{subfigure}[t]{0.45\textwidth}
		...
		\caption{An illustrative diagram of the proposed parametrisation for modelling the \(C_\alpha\) trace geometry of the cyclic peptide \textsc{c-AAGAGG}.}
	...
	\end{subfigure}
	\caption{Parametrising the conformational space of polypeptide backbones under anchor point distance constraints.}
...
To reduce the dimension of the problem, we only consider the \(C_\alpha\) traces (with fixed \(C_\alpha\)-\(C_\alpha\) link distances of 3.6 \AA) instead of the full polypeptide backbone (see \cref{fig:peptide_trace}), although we note that our framework is applicable to both settings.
