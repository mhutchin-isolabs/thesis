...
Finally in \cref{app:experimental_details}, we give a thorough description of experimental settings along with additional empirical results.
...
In particular, for any set of input $x = (x_1, \cdots, x_k) \in \c{X}^k$, we define the joint marginal as the solution of the following SDE \begin{equation}\label{eq:app_forward_SDE_sp} 
	\d\v{Y}_t = \frac{1}{2}\beta_t({m}(x) - \v{Y}_t) \d t + \sqrt{\beta_t K(x, x)}
\end{equation}
where \(m\) is a mean function and \(K\) a covaraince kernel.
...
\begin{table*}[] \centering \small \begin{tabular}{lllll} \toprule                 & No precond.
               ...
               Loss                             & $\|\sigma_{t|0}
               S^\top D_{\theta} + \v{z}\|_2^2$ & $\|\sigma_{t|0} D_{\theta} + S \v{z}\|_2^2$ & $\|\sigma_{t|0} D_{\theta} + \v{z}\|_2^2$ & $\|D_{\theta} - \v{Y}_0\|^2_2$      \\ $K \nabla \log p_t$ & $K D_\theta$ & $D_\theta$ & $ S D_\theta $ & $-\Sigma_{t|0}^{-1}(\v{Y}_t - \textrm{e}^{{-}\frac{B(t)}{2}} D_\theta)$ \\ \bottomrule\end{tabular} \caption{Summary of different score parametrisations as well as the values for $c_{\text{skip}}$ and $c_{\text{out}}$ that we found to be optimal, based on the recommendation from \textcite[Appendix B.6]{karras2022Elucidating}.
...
\Cref{tab:app:score-param} summarises the different options for parametrising the score as well as the values for $c_{\text{skip}}$ and $c_{\text{out}}$ that we found to be optimal, based on the recommendation from \textcite[Appendix B.6]{karras2022Elucidating}.
...
% This is actually a really strong constraint on the model class, and as soon as one goes beyond linearity (of the posterior with respect to.
% \ the context set), it is non trivial to enforce without directly parameterising a stochastic process, e.g.\ as extcite{phillips2022spectral}.
...
% Significant work has been done on performing convolutions on feature fields on generic manifolds (a superset of tensor fields on generic manifolds), core references being \cite{cohen2021equivariant} for the case of homogeneous spaces and \cite{weiler2023equivariant} for more general Riemannian manifolds.
...
\begin{proof}
	...
	Additionally assuming that $b_{\text{SDE}}$ and $\Sigma$ are bounded, \textcite[Proposition 3.6,][]{yim2023se} says that the distribution of $\bar{\v{Y}}_t$ is $G$-invariant, and in particular $\mathcal{L}(\bar{\v{Y}}_0)$.
...
\begin{theorem}[Invariant prior stochastic process implies an equivariant posterior map]
	\label{thm:equiv_posterior_maps}
	Using the language of \textcite{weiler2023equivariant} our tensor fields are sections of an associated vector bundle $\c{A}$ of a manifold $M$ with a $G$ structure.
	Let $\iso_{GM}$ be the group of $G$-structure preserving isometries on $M$.
	The action of this group on a section of the bundle $f \in \Gamma(\c{A})$ is given by \[ \phi \triangleright f := \phiA \circ f \circ \phi^{-1}.\]
...
% \begin{theorem}
% 	Let \(\del{X, \sigalg{X}}\) and \(\del{Y, \sigalg{Y}}\) be measurable spaces.
% 	Let \(\del{F, \sigalg{F}}\) be a measurable space of measurable function \(f:X\to Y\) with suitable regularity.
% 	Let \(G\) be a group and \(\triangleright_X: G\x X \to X\) a measurable action on \(X\).
% 	Assume that \(G\) has a measurable action \(\triangleright_F: G\x F \to F\) on \(F\) and a measurable action on \(Y\)dependent on \(X\) also \(\triangleright_Y: G\x X \x Y\to Y\).
% 	Assume that these actions are compatible in the sense that
% 	\[
% 		[g\triangleright_F f](x) = (g, x) \triangleright_Y f(g^{-1} \triangleright_X x).
% 	\]

% 	Define the function \(\eta: X^n \x F \to Y^n\) as \[
% 		\eta(\v{x}, f) = \eta(\sbr{x_1, ..., x_n}, f) = \sbr{f(x_1), ..., f(x_n)},
% 	\] with the shorthand \(\eta_{\v{x}}: F\to Y^n = \eta(\v{x}, \cdot)\) as the measurement of a function at a finite set of locations.
% 	Let \(\v{x} = \sbr{x_1, ..., x_n} \in X^n\), and let \(\v{y} = \sbr{y_1, ..., y_n} = \sbr{f(x_1), ..., f(x_n)}\).
% 	We can define a group action on this set of finite observations, \(\triangleright_{?}: G\x X^n\x Y^n \to X^n \x Y^n\) as
% 	\[
% 		g \triangleright_{?} (\v{x}, \v{y}) &= \del{\del{g \triangleright_X x_1, ..., g \triangleright_X x_n}, \del{(g, x_1)\triangleright_Y f(x_1), ..., (g, x_1)\triangleright_Y f(x_n)}} \\ &= \del{\del{g \triangleright_X x_1, ..., g \triangleright_X x_n}, \del{(g, x_1)\triangleright_Y y_1, ..., (g, x_1)\triangleright_Y y_n}}.
% 	\]

% 	Let the space of probability measures on \(F\) be denoted \(\Lambda_F\)
% 	Let \(\mu \in \Lambda_F\) be a probability measure on \(F\).
% 	Define the action of \(G\) on \(\Lambda_F\) via its action on measurable sets of \(F\),
% 	\[
% 		\sbr{g \triangleright_{\Lambda_F} \mu}(\c{F}) = \mu(g^{-1} \triangleright_F \c{F}) = \mu\del{\cbrcolon{g^{-1} \triangleright_F f}{f\in \c{F}}} \quad \text{ for } \; \c{F} \in \sigalg{F}, \; g\in G.
% 	\]

% 	Let \(\mu_{\v{x}} = \eta_{\v{x}}^* \mu\), the projection of the measure on \(F\) to measurement at a finite set of locations.
% 	Let \(\mu^{(\v{x}, \v{y})}\) be the unique measure satisfying
% 	\[
% 		\int_{Y^n} \int_F \phi(f) \d\mu^{(\v{x}, \v{y})}(f) \d \mu_{\v{x}}(\v{y})
% 	\] by disintegration, see \textcite{chang1997conditioning}.
% 	We shall term this the \emph{posterior measure}.
% 	Define the \emph{posterior map}, \(P: X^n \x Y^n \to \Lambda_F\), as
% 	\[
% 		P(\del{\v{x}, \v{y}}) = \mu^{\del{\v{x}, \v{y}}}.
% 	\]

% 	Assume \(\mu\) is invariant to the action of \(G\) in the sense that 
% 	\[
% 		g \cdot \mu = \mu.
% 	\]
% 	Then the posterior map is equivarant in the sense that
% 	\[
% 		P(g\triangleright_{?}\del{\v{x}, \v{y}}) = g \triangleright_{\Lambda_F} P(\del{\v{x}, \v{y}}).
% 	\]
% 	Equivalently
% 	\[
% 		\mu^{\del{\v{x}, \v{y}}}(\c{F}) = \mu^{g \triangleright_{?} \del{\v{x}, \v{y}}}(g\triangleright_F \c{F})\quad\forall g\in G \;, \c{F} \in \sigalg{F}.
% 	\]
% \end{theorem}
% \begin{proof}
% 	For a test function \(\phi\) we have that
% 	\[
% 		\int_F \phi(f) \d\mu(f) &= \int_{\cbrcolon{g \triangleright_F f'}{f \in F}} \phi(g \triangleright_F f') \d\mu(g \triangleright_F f') \marginnote{ by subsituting \(f = g \triangleright_F f'\)} \\
% 		&=\int_{F} \phi(g \triangleright_F f') \d\mu(g \triangleright_F f') \marginnote{by the closure of group actions} \\
% 		&=\int_{F} \phi(g \triangleright_F f') \d\mu(f') \marginnote{by the invariance assumption}.
% 	\]

% 	For all test functions \(\phi\) we have that
% 	\[
% 		\int_F \phi(f) \d\mu(f) = \int_{\eta_{\v{x}}F} \int_F \phi(f) \d\mu^{(\v{x}, \v{y})}(f) \d \mu_{\v{x}}(\v{y})
% 	\]
% \end{proof}

% We prove a few techincal lemmas first. 

% \begin{lemma}
% 	For a test function \(\phi\) we have that 
% 	\[
% 		\int_F \phi(f) \d \mu(f) = \int_F \phi(g \cdot f) \d \mu(f).
% 	\]
% \end{lemma}
% \begin{proof}
% 	\[
% 		\int_F \phi(f) \d \mu(f) &= \int_{g \cdot F} \phi(g \cdot f) \d \mu (g \cdot f) \\
% 		&= \int_F \phi(g \cdot f) \d \mu(f)
% 	\]
% 	due to the closure of the group action on 
% \end{proof}
...
\begin{proposition}
	...
	We have that $\lim_{t_0 \to 0} \pi_{t_0} = \pi_0$ where $\pi_0$ admits a density with respect to.
	the Lebesgue measure given by $x_0 \mapsto p(x_0|x_0^c)$.
\end{proposition}
...
\Cref{fig:1d_regression} shows samples for each of these dataset.
The Gaussian datasets are corrupted with observation noise with variance $\sigma^2=0.05^2$.
The left column of \cref{fig:1d_regression} shows example sample paths for each of the 5 datasets.
...
For {\scshape Sawtooth}, we obtained better performance with a wider and shallower model consisting of $2$ bi-dimensional attention layers with a hidden dimension of $128$.
...
\label{sec:ablation_cond_smapling_noise}
...
\begin{figure}
	\begin{subfigure}{\linewidth}
		\centering
		\includegraphics[width=\textwidth]{images/geomndp/1d/limiting_kernel_se_legend.pdf}\\\vspace{1mm}
		\includegraphics[width=\textwidth]{images/geomndp/1d/limiting_kernel_se.pdf}
		\caption{Squared Exponential dataset with lengthscale $\ell=0.25$}
	\end{subfigure}
	\begin{subfigure}{\linewidth}
		\centering
		\includegraphics[width=\textwidth]{images/geomndp/1d/limiting_kernel_periodic_legend.pdf}\\\vspace{1mm}
		\includegraphics[width=\textwidth]{images/geomndp/1d/limiting_kernel_periodic.pdf}
...
Another one with a $\groupE{2}$-equivariant score architecture, based on steerable CNNs~\parencite{thomas2018tensor,3DSteerable}.
...
Models are trained for $80k$ iterations, via \parencite{kingma2014adam} with a learning rate of $5e-4$ and a batch size of $32$.
