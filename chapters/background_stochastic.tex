...
Of particular relevance are \emphmarginnote{Borel \(\sigma\)-algebras}.
...
A \emphmarginnote{measure} on a measureable space is a function \(\mu: \sigalg{X} \to \R\) that is also non-negative, is zero on the empty set, and is countably additive.
...
Consider a joint random variable \(x,y: \Omega \to X \times Y\), a \(\c{F}/\sigalg{X}\otimes \sigalg{Y}\) measurable function, with distribution \(\pi_{x,y}:X \x Y \to \R\).

The \emphmarginnote{marginal random variable} \(x: \Omega \to X\) is given by \[\proj_x^*(,xy): \Omega \to X, \; \proj_x^*(x,y): \omega \mapsto (x, y) \mapsto x\], where the \(\proj_x\) function projects onto the \(x\) coordinate. 
We can create from the \emphmarginnote{marginal distribution} \(\pi_x:X\to\R\), given by \[\pi_x(A_x) = \pi_{x,y}(A_x\x Y), \; A_x\in\sigalg{X}\].

A \emphmarginnote{probability kernel} is a function \(\pi_{x|y}: X \times Y \to \R\) is a function that 1) \(\pi_{x|y}(\cdot \given y): X\to \R\) is a probability density and 2) \(\pi_{x|y} (A_x \given \cdot):Y\to\R\) is \(\sigalg{Y}/\c{B}(\R)\)-measureable. These are useful for a few reasons, defining stochastic processes, but also conditional distributions. 

We define the \emphmarginnote{conditional distribution}, \(\pi_{x|y}:X\times Y\to \R\) as the unique probability kernel satisfying
