...
The fact that \(\up{X}\) is measurable guarentees this is well defined.

\marginnote{The existence of random variables}To expand on the point of being able to choose \(\Omega\) to include enough events to be interesting, it is not guarenteed that for a given proabilty space \((\Omega, \c{F}, \P)\) and distribution \(\pi_\up{X}\) that there exists a random variable \(\up{X}: \Omega \to X\) such that \(\pi_\up{X} = \up{X}^*\P\). 
However, given a \(\pi_\up{X}\) and a proabilty space \((\Omega, \c{F}, \P)\) it is possible to find a \emph{bigger} probability space \((\Omega', \c{F}', \P')\) along with a measureable surjection \(i: \Omega' \to \Omega\) such that \(i_* \P' = \P\) and a random variable \(\up{X}: '\Omega \to X\) exists with distribution \(\pi_{\up{X}}\).
...
The \emphmarginnote{covariance} of a pair of random variables, if it exisits, is then given by \(\Cov\del{\up{X}, \up{X}'} = \E\sbr{\up{X}\up{X}'} - \E\sbr{\up{X}}E\sbr{\up{X}'}\).
...
A \emphmarginnote{probability kernel} is a function \(\pi_{\mathsf{X} | \mathsf{Y}}: \sigalg{X}\times Y \to \R\) is a function that 1) \(\pi_{\up{X}|\up{X}}(\cdot \given y): X\to \R\) is a probability measure and 2) \(\pi_{\up{X}|\up{X}} (A_x \given \cdot):Y\to\R\) is \(\sigalg{Y}/\c{B}(\R)\)-measureable. These are useful for a few reasons, defining stochastic processes, but also conditional distributions. 

We define the \emphmarginnote{conditional distribution}, \(\pi_{\up{X}|y}:X\times Y\to \R\), where it exists, as the unique probability kernel satisfying
...
\[ \d\Xf_t = \v{b}(\Xf_t, t)\d t + \sigma(t)\d \v{B}_t, \quad \Xf_0 \sim \pi\]
with time-marginal distributions \(p_t^\vsra{}(\v{X})\), the reverse-time stochastic differential equation
\[\d\Xb_t = \sbr{-\v{b}(\Xb_t, t) + \sigma(t)^2\nabla\log p_t(\Xb_t)}\d t + \sigma(t)\d \v{B}_t, \quad \Xb_0 \sim p_T,\] with time-marginal distributions \(p^\vsla{}_t(\v{X})\), the time-marginals are equal in the sense that \(p_t^\vsra{} = p_{T-t}^\vsla{} \;\forall\; t\in \sbr{0,T}\).
\end{theorem}
\begin{proof}
