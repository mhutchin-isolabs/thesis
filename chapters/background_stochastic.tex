...
For a countable sequence of spaces \((X_i, \sigalg{X_i})_{i=1}^\infty\) the \emphmarginnote[\baselineskip]{product \(\sigma\)-algebra} \(\bigotimes_i \sigalg{X_i}\) is given by the smallest \(\sigma\)-algebra containing the \emph{cylinder sets} (\cref{eq:cylendar_sets}) of the \(\sigalg{X_i}\).
...
For \emphmarginnote{measurable functions on product spaces} the function \(f: X\to Y\times Y'\) is \(\sigalg{X}/\sigalg{Y}\otimes\sigalg{Y'}\)-measurable if each component function is measurable. The converse is \emph{not} true.  \(f: X\times X'\to Y\) is not necessarily measurable if \(f(\cdot, x'): X\to Y\) and \(f(x, \cdot): X'\to Y\) are measurable.
...
This definition of expectation applies clearly to real vector-valued random variables as well.
...
Such an \(f\) if called the \emphmarginnote{Radon-Nikodym derivative} of \(\mu\) with respect to \(\nu\), and is often denoted as \(\od{\mu}{\nu}\).
...
The existence of Radon-Nikodym derivates lets us connect the measure theory formalisation back to the more comment density based formulation.
We say that a distribution \(\pi:X\to \R\) has a \emphmarginnote{distribution with respect a measure} \(\lambda:X\to\R\) if its Radon-Nikodym derivative exists, i.e. \(\pi << \lambda\).
...
We say that a stochastic process \(\del{X_t}_{t\geq 0}\) on a measurable space \((X, \sigalg{X})\) with a probability space \((\Omega, \c{F}, \P)\) is \emphmarginnote{adapted to a filtration\label{def:adapted_process}} \(\del{\c{F}_t}_{t\geq 0}\) if for all \(t\), \(X_t: \Omega \to X\) is an \(\c{F}_t/\sigalg{X}\)-measurable function.
...
Using the Kolmogorov continuity theorem we pick the Brownian motion that has continuous paths.
...
Using this trick we can easily map between the notational convenience of an explicit time dependence, and the technical convenience of time-homogenous coefficients.
...
For a stochastic differential equation of the form \[\d X_t = b(t, X_t) \d t + \sigma(t, X_t) \circ_\alpha \d B_t,\] 
 with solution \(\del{X_t}_{t \geq 0}\) 
...
\subsection{Multi-dimension stochastic differential equations}
...
The change of variable formula can also be extended to the multi-dimension and any \(\alpha\) setting.
...
\end{proof}
Note how we are differentiating with respect to the time of the \emph{initial condition} of the diffusion we specify, not the time at which we evaluate the expectation.
...
We can define the \emph{generator} of a \emph{time-homogenous} diffusion, one for which the diffusion coefficients are independent of time, as
...
\end{align}
Taking the negative of this term we see that the evolution of the time-marginal density of a stochastic differential equation, but with the evolution happing in reverse, is given by
\[
...
with an initial condition \(\v{X}_0\), the most simple approximate solution is \emphmarginnote{Euler integration}.
...
with initial condition \(\v{X}_0\), we can apply the \emphmarginnote{Euler-Maruyama integration} to approximate the solution.
...
As such, the Euler-Maruyama scheme \emphhyperref{converges in distribution}{def:distribution-converge-rv} to the solution to the original stochastic differential equation as we decrease step size of the solution.
