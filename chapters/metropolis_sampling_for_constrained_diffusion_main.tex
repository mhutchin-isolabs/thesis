...
% \textcite{lou2023reflected} have investigated the use of reflected diffusion models for image modelling.
...
% However, the introduced samplers suffer from the same limitations as the previous method for more complex manifolds and constraints than the hypercube.
...
Briefly, that given a Riemannian manifold \((\c{N}, {h})\), we consider a family of real functions \(\{f_i: \c{N} \to \R\}_{i \in \mathcal{I}}\) indexed by \(\mathcal{I}\).
...
\begin{table*}[t]
	...
	% to improve formatting we use \thead for non-block-aligned line breaks
	% in the header, as that's automatically set to \small we adjust
	% the rest of the table to small as well and then rescale
	% it to textwidth
	\vspace{1em}
	\begin{tabular}{lcccc}
		& \multicolumn{2}{c}{\(\overbracket{\mspace{200mu}}^{\text{\small Both required for fast DSM loss}}\)} &  & \\ %\multicolumn{2}{c}{\(\overbracket{\mspace{200mu}}^{\text{\small Preserves inductive biases}}\)} \\
		\toprule
...
% \begin{definition}[Metropolis approximation of reflect Brownian motion]
...
As in \cref{cpt:constrained_domains}, we note it is possibly to infinitesimally smooth boundaries to match the technical setting with very minimal practical impact.
...
In~\cref{sec:exp_synthetic_tasks}, we assess the scalability of our method by applying it to synthetic distributions on hypercubes and simplices of increasing dimensionality, similar to \cref{subsec:synthetic_data}.
...
A quantitative comparison of the log-likelihood of a held-out test set is presented in~\cref{tab:synthetic_polytope_dupref}, while a visual comparisons of the fitted distributions are shown in \cref{fig:app:synthetic_hypercube,fig:app:synthetic_simplex}.
...
We observe a similar difference in the scaling properties of reflected and Metropolis models when measuring the convergence times of the respective forward noising processes to the uniform distribution on hypercubes \([-1, 1]^d\) and simplices \(\Delta^{d}\) of increasing dimensionality. The results are presented in~\cref{fig:forward_process_timing} and show that the convergence time of the Metropolis process scales linearly in the dimension, while the reflected process scales quadratically.
...
\begin{figure}[t]
	...
	\caption{A qualitative comparison of \(10^5\) samples from the data distribution, our Metropolis diffusion model, a reflected diffusion model and the uniform distribution for the constrained modelling of manipulability ellipsoids of robotic arms.
		For visual clarity, the figures show the SPD matricies plotted .
	}
...
To this end, we derived a geospatial dataset based on wildfire incidence rates within the continental United State  and train a Metropolis diffusion model constrained by the corresponding country borders on the sphere \(\c{S}^2\).

\subsubsection{Dataset}
\label{sec:wildfire_data_dataset}
To create the datset we retrieved the rasterised version of the wildfire data provided by \textcite{welty2020combined}.
...
A visualization of the resulting dataset is presented in~\cref{fig:wildfire_data_dist}.

\subsubsection{Point-in-spherical-polytope algorithms}
\label{sec:wildfire_data_algo}

The support of the data-generating distribution we aim to approximate is restricted to a highly non-convex spherical polytope \(\mathbb{P}\in\c{S}^2\) given by the country borders of the continental United States.
...
This demonstrates that our approach is able to successfully model challenging multimodal and sparse distributions on constrained manifolds with highly non-convex boundaries.

\begin{table}[h]
    \setlength{\tabcolsep}{8pt}
    % align it with top of paragram by subtracting default padding
