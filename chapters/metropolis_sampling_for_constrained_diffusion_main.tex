...
In this chapter, we propose a new method for generative modelling on constrained manifolds based on a Metropolis-based discretisation of the reflected Brownian motion.
The Metropolised process' chief advantage is that it is lightweight: the only additional requirement over those outlined in \cref{chp:riemannian_score_based_models} that is needed to implement a constrained diffusion model is an efficient binary function that indicates whether any given point is within the constrained set.
...
% \textcite{debortoli2022riemannian}
...
This definition ensures that the barrier function induces a well-defined exponential map on the manifold, making the Riemannian diffusion model frameworks of \textcite{debortoli2022riemannian} and \textcite{huang2022riemannian} applicable.
...
\begin{algorithm}[H]
	\caption{\emph{Metropolis approx. of refelcted Brownian motion}}
...
\begin{table*}[t]
	...
	\begin{tabular}{lccc}
		\toprule
		\thead[l]{\scshape Sampling Method}                                  & \thead[c]{\scshape Set Indicator \\Function} & \thead[c]{\scshape Intersection\\Operator} & \thead[c]{\scshape Distance to\\Boundary} \\
		\midrule
		\scshape Barrier method (\Cref{cpt:constrained_domains})             & \cmark                  & \cmark & \cmark \\
		\scshape Reflected (\Cref{cpt:constrained_domains}, \textcite{lou2023reflected}) & \cmark                  & \cmark & \xmark \\
