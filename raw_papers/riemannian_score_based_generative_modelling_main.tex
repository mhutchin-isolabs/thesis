...
A simple choice is to use \emphmarginnote{Langevin dynamics} described by \begin{equation} \label{eq:langevin} \mathrm{d} \v{X}_t = - \tfrac{1}{2}~\nabla_{\v{X}_t} U(\v{X}_t) \mathrm{d} t + \mathrm{d} \v{B}_t^\c{M}, \end{equation} which admits the invariant density (w.r.t. \ the volume form) given by $\mathrm{d} p_{\textup{inv}}/ \mathrm{d} \textrm{Vol}_\c{M}(x) \propto \mathrm{e}^{-U(x)}$ \parencite[Section 2.4]{durmus2016high}, where $\nabla$ is the Riemannian gradient\footnote{The (Riemannian) gradient $\nabla$ is defined s.t.
