...
This process is also a diffusion whose drift depends on the logarithmic gradients of the noised data densities, i.e. the Stein scores, estimated using a neural network via score matching \parencite{hyvarinen2005estimation,vincent2011connection}.
...
\section{Euclidean Score-Based Generative Modelling} \label{sec:eucl-sgm-riem} We recall here briefly the key concepts behind SGMs on the Euclidean space $\R^d$ and refer the readers to \textcite{song2020score} for a more detailed introduction.
We consider a forward \emphmarginnote{noising process} $(\v{X}_t)_{t \geq 0}$ defined by the following Stochastic Differential Equation (SDE) \begin{equation}\label{eq:forward_SDE} \mathrm{d} \v{X}_t = -\v{X}_t \mathrm{d} t + \sqrt{2} \mathrm{d} \v{B}_t,\quad \v{X}_0 \sim p_0 , \end{equation} where $(\v{B}_t)_{t \geq 0}$ is a $d$-dimensional Brownian motion and $p_0$ is the data distribution.
The available data gives us an empirical approximation of $p_0$.
The process $(\v{X}_t)_{t \geq 0}$ is simply an Ornstein--Ulhenbeck (OU) process which converges with geometric rate to $\c{N}(0,\m{I})$.
Under mild conditions on $p_0$, the \emphmarginnote{time-reversed process} $(\v{Y}_t)_{t \geq 0} = (\v{X}_{T-t})_{t \in \sbr{0,T}}$ also satisfies an SDE \parencite{cattiaux2021time,haussmann1986time} given by \begin{equation} \label{eq:backward_SDE} \mathrm{d} \v{Y}_t = \cbr{ \v{Y}_t + 2 \nabla \log p_{T-t}(\v{Y}_t)} \mathrm{d} t + \sqrt{2} \mathrm{d} \v{B}_t,\quad \v{Y}_0 \sim p_T , \end{equation} where $p_t$ denotes the density of $\v{X}_t$.
By construction, the law of $\v{Y}_{T-t}$ is equal to the law of $\v{X}_t$ for $t \in \sbr{0,T}$ and in particular $\v{Y}_{T}\sim p_0$.
Hence, if one could sample from $(\v{Y}_t)_{t \in \sbr{0,T}}$ then its final distribution would be the data distribution $p_0$.
Unfortunately we cannot sample exactly from (\ref{eq:backward_SDE}) as $p_T$ and the \emphmarginnote{stein scores} $(\nabla \log p_t(x))_{t\in[0,T]}$ are intractable.
Hence SGMs rely on a few approximations.
First, $p_T$ is replaced by the reference distribution $\c{N}(0,\m{I})$ as we know that $p_T$ converges geometrically towards it.
Second, the following \emphmarginnote{denoising score matching} identity is exploited to estimate the scores \begin{equation}\label{eq:scoreidentity} \nabla_{x_t} \log p_t(x_t) = \int_{\R^d} \nabla_{x_t} \log p_{t|0}(x_t|x_0)~p_{0|t}(x_0|x_t) \mathrm{d} x_0 \end{equation} where $p_{t|0}(x_t|x_0)$ is the transition density of the OU process (\ref{eq:forward_SDE}) which is available in closed-form.
It follows directly that $\nabla \log p_t$ is the minimizer of $\ell_t(\mathbf{s}) = \E\sbr{\norm{\mathbf{s}(\v{X}_t) - \nabla_{x_t} \log p_{t|0}(\v{X}_t|\v{X}_0)}^2}$ over functions $\mathbf{s}$ where the expectation is over the joint distribution of $\v{X}_0,\v{X}_t$.
This result can be leveraged by considering a neural network $\mathbf{s}_\theta: \sbr{0,T} \times \R^d \to \R^d$ trained by minimizing the loss function $\ell(\theta)=\int_0^T \lambda_t \ell_{t}(\mathbf{s}_\theta(t,\cdot))\mathrm{d} t$ for some weighting function $\lambda_t>0$.
Finally, an \emphmarginnote{Euler--Maruyama discretization} of (\ref{eq:backward_SDE}) is performed using a discretization step $\gamma$ such that $T=\gamma N$ for $N \in \N$ \begin{align} \label{eq:backward_discrete_final} Y_{n+1} = Y_n + \gamma \{Y_n + 2 \mathbf{s}_\theta(T -n \gamma, Y_n) \} + \sqrt{2 \gamma} Z_{n+1}, \\ Y_0\sim \mathrm{N}(0,\m{I}), \quad Z_n \overset{\textup{i.i.d.
		}}{\sim} \c{N}(0,\m{I}). \nonumber
\end{align}
The above showcases the basics of SGMs but we highlight that many improvements have been proposed; see e.g.~\textcite{song2020improved,jolicoeur2020adversarial,nichol2021beatgans}.
In particular, selecting an adaptive stepsize $(\gamma_n)_{n \in \N}$ \parencite{bao2022analyticdpm,watson2021learning} and using a predictor-corrector scheme \parencite{song2020score} instead of a simple Euler--Maruyama discretization drastically improves performance.

\section{Riemannian Score-Based Generative Modelling}

\label{sec:score-appr-manif}
We now move to the Riemannian manifold setting, and more specifically assume that $\c{M}$ is a complete, orientable connected and boundaryless Riemannian manifold, endowed with a Riemannian metric $g$~\footnote{ Metrics $g$ are sections of $\mathrm{T}^* \c{M} \otimes \mathrm{T}^* \c{M}$, the rank 2 tensor bundle of the dual tangent space, i.e.\ smooth varying bilinear maps on $\mathrm{T} \c{M}$, verifying symmetry and positive semi-definiteness.
}.
Four components are required to extend SGMs to this setting: \begin{enumerate*}[label=\roman*)] \item a forward \emph{noising} process on $\c{M}$ which converges to an easy-to-sample reference distribution, \item a time-reversal formula on $\c{M}$ which defines a backward generative process, \item a method for approximating samples of SDEs on manifolds, \item a method to efficiently approximate the drift of the time-reversal process.
\end{enumerate*}
Notation are gathered in \Cref{sec:notation-1}.
...
A simple choice is to use \emphmarginnote{Langevin dynamics} described by \begin{equation} \label{eq:langevin} \mathrm{d} \v{X}_t = - \tfrac{1}{2}~\nabla_{\v{X}_t} U(\v{X}_t) \mathrm{d} t + \mathrm{d} \v{B}_t^\c{M}, \end{equation} which admits the invariant density (w.r.t.
\ the volume form) given by $\mathrm{d} p_{\textup{inv}}/ \mathrm{d} \textrm{Vol}_\c{M}(x) \propto \mathrm{e}^{-U(x)}$ \parencite[Section 2.4]{durmus2016high}, where $\nabla$ is the Riemannian gradient\footnote{The (Riemannian) gradient $\nabla$ is defined s.t.
	\ for any $f:\c{M} \rightarrow \R$, $x \in \c{M}, v \in \mathrm{T}_x\c{M}$, $\langle \nabla f, v \rangle_g = \mathrm{d} f (v)$.}.

Two simple choices for $U(x)$ present themselves.
Firstly, setting $U(x) = d_\c{M}(x, \mu)^2/(2 \gamma^2)$, where $d_\c{M}$ is the geodesic distance and $\mu \in \c{M}$ is an arbitrary mean location, induces the drift $\nabla_{\v{X}_t} U(\v{X}_t) = -\exp^{-1}_{\v{X}_t}(\mu) /\gamma^2$~\footnote{ $\exp_x: \mathrm{T}_x \c{M} \to \c{M}$ denotes the exponential mapping on the manifold, see e.g. \textcite[Chapter 20]{lee2013smooth}.
}.
This is the potential of the \emphmarginnote{Riemannian normal} \parencite{pennec2006Intrinsic} distribution, An alternative is to target the \emphmarginnote{exponential wrapped Gaussian}.
This is the pushforward of a Gaussian distribution in the tangent space at the mean location along the exponential map.
The potential is given by $U(x) = d_{\c{M}}(x, \mu)^2/(2 \gamma^2) + \log |D \exp^{-1}_{\mu}(x)|$\footnote{$|\cdot|$ denotes the absolute value of the determinant, and $\partial f$ the Jacobian of $f$.
}.
In contrast to the Riemannian normal, sampling and evaluating the density of this distribution is easy.
The advantage of this process is that sampling for this exp-wrapped normal distribution is trivial and so is computing the log-likelihood of points under it.
...
This result shows that if $(\v{X}_t)_{t \in \sbr{0,T}}$ is a diffusion process then $(\v{X}_{T-t})_{t \in \sbr{0,T}}$ is also a diffusion process w.r.t.
the backward filtration whose coefficients can be computed, and are shown in \cref{eq:time_reversal_manifold}.
...
\begin{theorem}[Manifold time-reversed process]
	...
	In addition, assume that $\mathbb{P}_t=\mathcal{L}(\v{X}_t)$, the distribution of $\v{X}_t$, admits a smooth positive density $p_t$ w.r.t.
	\ $p_{\textup{inv}}$ for any $t \in \sbr{0,T}$.
...
The loss $\ell^{\textup{im}}_t$ is referred to as the \emph{implicit} score matching (ISM) loss~\parencite{hyvarinen2005estimation}.
...
For any $x_0 \in \c{M}$ and $t \geq s \geq 0$, the heat kernel $p_{t|s}(\cdot|x_s)$ is defined as the density of $\v{B}_t^\c{M}$ w.r.t.
the uniform measure on the manifold.
...
\begin{table*}[t]
	...
	\begin{tabular}{lrrrrr}
		...
		Riemannian CNF            & $\bm{-6.05_{\pm 0.61}}$                 & ${0.14_{\pm 0.23}}$                       & ${1.11_{\pm 0.19}}$                      & $\v{-0.80_{\pm 0.54}}$                    \\
		Moser Flow                & ${-4.21_{\pm 0.17}}$                    & $\v{-0.16_{\pm 0.06}}$                    & $\v{0.57_{\pm 0.10}}$                    & $\v{-1.28_{\pm 0.05}}$                    \\
		Stereographic Score-Based & \cellcolor{blue!20}${-3.80_{\pm 0.27}}$ & \cellcolor{blue!20}$\v{-0.19_{\pm 0.05}}$ & \cellcolor{blue!20}$\v{0.59_{\pm 0.07}}$ & \cellcolor{blue!20}$\v{-1.28_{\pm 0.12}}$ \\
		Riemannian Score-Based    & \cellcolor{blue!20}${-4.92_{\pm 0.25}}$ & \cellcolor{blue!20}$\v{-0.19_{\pm 0.07}}$ & \cellcolor{blue!20}$\v{0.45_{\pm 0.17}}$ & \cellcolor{blue!20}$\v{-1.33_{\pm 0.06}}$ \\
		\midrule
